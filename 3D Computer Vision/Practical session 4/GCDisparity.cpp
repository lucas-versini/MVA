...
double rho(double x) {
    if (x <= 0) return 1.;
    else if (x <= 1) return sqrt(1 - x);
    else {
        std::cerr << "Error in the argument for rho." << std::endl;
        return 0.;
    }
}

// Index of the node (x, y, d)
...
void build_graph(Graph<int,int,int>& G,
    ...
    for (int x = 0; x < nx; x++){
        for (int y = 0; y < ny; y++){
            // Compute K based on the number of neighbors
            // See Section 2 of the report
            int neighbors = 4;
            if (x == 0 || x == nx - 1) neighbors -= 1;
            if (y == 0 || y == ny - 1) neighbors -= 1;

            int K = 1 + nd * neighbors * lambda;

            // Zoomed coordinates
            int a = win + zoom * x, b = win + zoom * y;

            // Check that there exists a valid patch for the current pixel
            if (a + dmax < win || a + dmin >= I2.width() - win) continue;

            for (int d = 0; d <= nd; d++){
                int w;
                if (win <= a + (dmin + d) && a + (dmin + d) < I2.width() - win)
                    w = K + int(wcc * rho(zncc(I1, I1M, I2, I2M, a, b, a + (dmin + d), b)));
                else
                    w = INF;

                int i = index(x, y, d - 1, nx, ny, nd), j = index(x, y, d, nx, ny, nd);

                // If d == nd, we only add one t-link (sink).
                // If d == 0, one t-link (source) and n-links.
                // Otherwise, only n-links.
                if (d == 0) G.add_tweights(j, w, 0);
                else if (d == nd) G.add_tweights(i, 0, w);
                else G.add_edge(i, j, w, 0);

                if (d != nd){
                    if (x + 1 < nx)
                        G.add_edge(index(x + 1, y, d, nx, ny, nd), j, lambda, lambda);
                    if (y + 1 < ny)
                        G.add_edge(index(x, y + 1, d, nx, ny, nd), j, lambda, lambda);
                }
            }
        }
    }
...
doubleImage decode_graph(Graph<int,int,int>& G, int nx, int ny, int nd, int w2){
    ...
    for (int x = 0; x < nx; x++){
        for (int y = 0; y < ny; y++){
            int a = win + zoom * x;
            if (a + dmin >= w2 - win || a + dmax < win){
                // No valid patch for this pixel
                D(x, y) = dmin - 1;
            }
            else {
                // Count the number of cuts to find the disparity
                D(x, y) = dmin - 1;
                for (int d = 0; d < nd; d++){
