...
def load_dataset():
    ...
    Gs = [to_networkx(data, node_attrs = None, edge_attrs = None, to_undirected = True) for data in dataset]
    ##################
...
def graphlet_kernel(Gs_train, Gs_test, n_samples=200):
    ...
    for i, graph in enumerate(Gs_train):
        for _ in range(n_samples):
            s = np.random.choice(graph.nodes(), 3, replace = False)
...
print(f"Accuracy of the shortest path kernel: {accuracy_score(y_test, ypred_sp)}.")

# Graphlet kernel
clf_g = SVC(kernel = 'precomputed')
clf_g.fit(K_train_g, y_train)
ypred_g = clf_g.predict(K_test_g)
print(f"Accuracy of the graphlet kernel: {accuracy_score(y_test, ypred_g)}.")
##################
