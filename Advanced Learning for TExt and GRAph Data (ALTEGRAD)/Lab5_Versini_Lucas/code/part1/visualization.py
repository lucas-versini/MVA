...
def visualize(model, n, dim):
    ...
    ##################
    frequencies = [model.wv.get_vecattr(node, "count") for node in nodes] # Frequencies of each node
    embeddings = [model.wv[nodes[idx]] for idx in np.argsort(frequencies)[-100:]] # Embeddings of the 100 most frequent nodes
    DeepWalk_embeddings = np.array(embeddings)
    ##################
